// `@app/demo-thing` — the module's own test, against `createTestKernel` (**R4**).
//
...
import { DemoThingGatewayToken, DemoThingServiceToken, DemoThingModule } from './contract';
import { acceptHotUpdate, module } from './module';
import type { ModuleHotContext } from './module';

describe('demo-thing module', () => {
  it('D1 / criterion 9: nothing is evaluated until the activation trigger, then in order', async () => {
    ...
    expect(kernel.status(DemoThingModule)).toBe('registered');
    ...
    await kernel.activate(DemoThingModule);
    ...
    expect(evaluationLog(kernel).map((event) => event.file)).toEqual([
      '<activate>',
      '<providers>',
      'demo-thing/providers.ts',
      '<init>',
      'demo-thing/lifecycle.ts',
    ]);
  ...
  it('R4: activates with a mocked gateway, behaves, and disposes without leaks', async () => {
    ...
    await kernel.activate(DemoThingModule);
    expect(kernel.status(DemoThingModule)).toBe('ready');

    const service = kernel.get(DemoThingServiceToken);
    ...
    expect([...owners]).toEqual(['demo-thing']);
  ...
  describe('H2: acceptHotUpdate', () => {
    ...
    it('an update carrying a descriptor calls hotReplace and re-arms the fresh copy', () => {
      ...
      expect(replaced).toEqual([{ ref: 'demo-thing', next: 'demo-thing' }]);

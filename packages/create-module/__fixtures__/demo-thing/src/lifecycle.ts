// `@app/demo-thing` — the module lifecycle (spec §8).
//
...
import type { DemoThingRecord } from './contract';

// **Acceptance criterion 9** — see the same call in `providers.ts`.
recordEvaluation('demo-thing', 'demo-thing/lifecycle.ts');
 ...
 * factory reached from here is `'demo-thing'`.
 */
export function init(ctx: ModuleContext): void {
  const service = ctx.get(DemoThingServiceToken);
  const drafts = ctx.get(DemoThingDraftsToken);
  ...
  ctx.on(service, 'demo-thing/changed', (record: DemoThingRecord) => {
  ...
  ctx.effect(() => {
    const session = ctx.get(DemoThingSessionToken);

// `@app/demo-thing` — provider declarations (spec §7.2).
//
...
//
//  - `DemoThingSettingsToken` — a plain data provider, the thing a test or
//    the composition root replaces with `override: true` (**C6**).
//  - `DemoThingGatewayToken` — the module's remote edge, and the token the
//    emitted test mocks.
//  - `DemoThingServiceToken` — `scope: 'module'` (**C2**), with `MODULE_ID`
//    among its deps (**C4**) and a `dispose()` the container calls when the
//    module is disposed (**C7**).
//  - `DemoThingDraftsToken` — `persistent: true` over `defineStore`
...
import {
  DemoThingDraftsToken,
  DemoThingGatewayToken,
  DemoThingServiceToken,
  DemoThingSessionToken,
  DemoThingSettingsToken,
} from './contract';
import type {
  DemoThingDraft,
  DemoThingGateway,
...
recordEvaluation('demo-thing', 'demo-thing/providers.ts');

function createDemoThingGateway(settings: DemoThingSettings): DemoThingGateway {
  return {
    fetch: async (id) => {
      if (!settings.enabled) {
        throw new Error(`demo-thing: disabled by settings, cannot fetch '${id}'.`);
      }
      return { id, label: `DemoThing ${id}` };
    },
  };
}

function createDemoThingService(
  gateway: DemoThingGateway,
  settings: DemoThingSettings,
  requester: string,
): DemoThingService {
  const handlers = new Set<(record: DemoThingRecord) => void>();
  return {
    load: async (id) => {
      ...
      throw new Error(`demo-thing: load('${id}') failed for '${requester}'.`, { cause: lastError });
    },
    on: (_event, handler) => {
...
function openDemoThingSession(service: DemoThingService): DemoThingSession {
  let open = true;
  return {
    flush: async () => {
      if (open) {
...
export const providers: AnyProviderRecord[] = [
  provide(DemoThingSettingsToken, {
    // C2: `singleton` is the default scope and is what a value with no
    // per-module identity wants.
    factory: () => ({ enabled: true, maxRetries: 2 }),
  }),

  provide(DemoThingGatewayToken, {
    deps: [DemoThingSettingsToken],
    factory: (settings) => createDemoThingGateway(settings),
  }),

  provide(DemoThingServiceToken, {
    ...
    deps: [DemoThingGatewayToken, DemoThingSettingsToken, MODULE_ID],
    factory: (gateway, settings, requester) =>
      createDemoThingService(gateway, settings, requester),
  }),

  provide(DemoThingSessionToken, {
    ...
    scope: 'transient',
    deps: [DemoThingServiceToken],
    factory: (service) => openDemoThingSession(service),
  }),

  provide(DemoThingDraftsToken, {
    ...
    scope: 'module',
    persistent: true,
    factory: () => defineStore<readonly DemoThingDraft[]>([]),
  }),

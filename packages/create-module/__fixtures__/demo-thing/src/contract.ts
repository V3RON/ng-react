// `@app/demo-thing/contract` — the module's public surface (spec §4).
//
...
export const DemoThingModule = moduleRef('demo-thing');

/** Plain, immutable configuration — the shape a composition root or a test overrides. */
export interface DemoThingSettings {
  readonly enabled: boolean;
  readonly maxRetries: number;
}

/** One record this module owns. */
export interface DemoThingRecord {
  readonly id: string;
  readonly label: string;
}

/** A not-yet-committed edit. Durable across HMR — see `DemoThingDraftsToken`. */
export interface DemoThingDraft {
  readonly id: string;
 ...
 * `provide(DemoThingGatewayToken, { override: true, … })` — spec §15
 * criterion 7's "activate with a mocked gateway" in this module's terms.
 */
export interface DemoThingGateway {
  fetch(id: string): Promise<DemoThingRecord>;
}
 ...
 */
export interface DemoThingService {
  load(id: string): Promise<DemoThingRecord>;
  on(event: string, handler: (record: DemoThingRecord) => void): void;
  off(event: string, handler: (record: DemoThingRecord) => void): void;
 ...
 * a component (**R2**).
 */
export interface DemoThingSession {
  flush(): Promise<void>;
  close(): void;
}

/** **C1**: token labels are `moduleId/Name` (ADR-8). Each call is a distinct identity. */
export const DemoThingSettingsToken = createToken<DemoThingSettings>('demo-thing/Settings');
export const DemoThingGatewayToken = createToken<DemoThingGateway>('demo-thing/Gateway');
export const DemoThingServiceToken = createToken<DemoThingService>('demo-thing/Service');
export const DemoThingSessionToken = createToken<DemoThingSession>('demo-thing/Session');
/** **H3**: durable state lives in a store, never in a module closure. */
export const DemoThingDraftsToken =
  createToken<Store<readonly DemoThingDraft[]>>('demo-thing/Drafts');

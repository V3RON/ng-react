// `__pkg__/module` — the kernel-facing descriptor (spec §5.2).
//
// **B1**: this entry point may be imported only by the application's
// composition root; every other importer uses `__pkg__/contract`.
//
...
// __dependsOnImports__
import { __Ref__ } from './contract';
...
interface __Pascal__ModuleExports {
 ...
 * lifecycle and provider chunks". It cannot: a kernel module id (`'__id__'`)
 ...
 * the kernel and already imports `__pkg__/module`, calls this once:
 *
 * ```ts
 * import { module as __camel__Module, acceptHotUpdate } from '__pkg__/module';
 * const kernel = createKernel({ modules: [__camel__Module, …] });
...
export function acceptHotUpdate(
  ...
  hot.accept((next) => {
    const replacement = next as __Pascal__ModuleExports | undefined;
    ...
    void kernel.hotReplace(__Ref__, replacement.module);

...
interface DashboardModuleExports {
 ...
 * lifecycle and provider chunks". It cannot: a kernel module id (`'dashboard'`)
 ...
 * the kernel and already imports `@app/dashboard/module`, calls this once:
 *
 * ```ts
 * import { module as dashboardModule, acceptHotUpdate } from '@app/dashboard/module';
 * const kernel = createKernel({ modules: [dashboardModule, …] });
...
export function acceptHotUpdate(kernel: Kernel, hot?: ModuleHotContext): void {
  ...
  context.accept((next) => {
    const replacement = next as DashboardModuleExports | undefined;
    ...
    void kernel.hotReplace(DashboardModule, replacement.module);

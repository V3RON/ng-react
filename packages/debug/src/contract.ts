...
/** A not-yet-committed edit. Durable across HMR — see `DebugDraftsToken`. */
...
export interface DebugService {
  load(id: string): Promise<DebugRecord>;
  on(event: string, handler: (record: DebugRecord) => void): void;
  off(event: string, handler: (record: DebugRecord) => void): void;
...
export const DebugSettingsToken = createToken<DebugSettings>('debug/Settings');
export const DebugGatewayToken = createToken<DebugGateway>('debug/Gateway');
export const DebugServiceToken = createToken<DebugService>('debug/Service');
export const DebugSessionToken = createToken<DebugSession>('debug/Session');
/** **H3**: durable state lives in a store, never in a module closure. */
export const DebugDraftsToken =
  createToken<Store<readonly DebugDraft[]>>('debug/Drafts');

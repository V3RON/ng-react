// `@app/debug/module` — the kernel-facing descriptor (spec §5.2).
//
// **B1**: this entry point may be imported only by the application's
// composition root; every other importer uses `@app/debug/contract`.
//
...
import { DebugModule } from './contract';
 ...
 * lifecycle and provider chunks". It cannot: a kernel module id (`'debug'`)
 ...
 * the kernel and already imports `@app/debug/module`, calls this once:
 *
 * ```ts
 * import { module as debugModule, acceptHotUpdate } from '@app/debug/module';
 * const kernel = createKernel({ modules: [debugModule, …] });
...
export function acceptHotUpdate(kernel: Kernel, hot?: ModuleHotContext): void {
  ...
  context.accept((next) => {
    const replacement = next as DebugModuleExports | undefined;
    ...
    void kernel.hotReplace(DebugModule, replacement.module);

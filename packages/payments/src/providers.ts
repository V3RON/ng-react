...
//
//  - `PaymentsSettingsToken` — a plain data provider, the thing a test or
//    the composition root replaces with `override: true` (**C6**).
//  - `PaymentsGatewayToken` — the module's remote edge, and the token the
//    emitted test mocks.
//  - `PaymentsServiceToken` — `scope: 'module'` (**C2**), with `MODULE_ID`
//    among its deps (**C4**) and a `dispose()` the container calls when the
//    module is disposed (**C7**).
//  - `PaymentsDraftsToken` — `persistent: true` over `defineStore`
...
import {
  PaymentsDraftsToken,
  PaymentsGatewayToken,
  PaymentsServiceToken,
  PaymentsSessionToken,
  PaymentsSettingsToken,
} from './contract';
import type {
  PaymentsDraft,
  PaymentsGateway,
...
function createPaymentsGateway(settings: PaymentsSettings): PaymentsGateway {
  return {
    fetch: async (id) => {
      if (!settings.enabled) {
        throw new Error(`payments: disabled by settings, cannot fetch '${id}'.`);
      }
      return { id, label: `Payments ${id}` };
    },
  };
}
...
function createPaymentsService(
  ...
  const handlers = new Set<(record: PaymentsRecord) => void>();
  return {
    load: async (id) => {
      ...
      throw new Error(`payments: load('${id}') failed for '${requester}'.`, { cause: lastError });
    },
    on: (_event, handler) => {
...
function openPaymentsSession(service: PaymentsService): PaymentsSession {
  let open = true;
  return {
    flush: async () => {
      if (open) {
...
export const providers: AnyProviderRecord[] = [
  ...
  provide(PaymentsGatewayToken, {
    deps: [PaymentsSettingsToken],
    factory: (settings) => createPaymentsGateway(settings),
  }),

  provide(PaymentsServiceToken, {
    ...
    deps: [PaymentsGatewayToken, PaymentsSettingsToken, MODULE_ID],
    factory: (gateway, settings, requester) =>
      createPaymentsService(gateway, settings, requester),
  }),

  provide(PaymentsSessionToken, {
    ...
    scope: 'transient',
    deps: [PaymentsServiceToken],
    factory: (service) => openPaymentsSession(service),
  }),

  provide(PaymentsDraftsToken, {
    ...
    scope: 'module',
    persistent: true,
    factory: () => defineStore<readonly PaymentsDraft[]>([]),
  }),

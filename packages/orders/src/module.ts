// `@app/orders/module` — the kernel-facing descriptor (spec §5.2).
//
// **B1**: this entry point may be imported only by the application's
// composition root; every other importer uses `@app/orders/contract`.
//
 ...
 * lifecycle and provider chunks". It cannot: a kernel module id (`'orders'`)
 ...
 * the kernel and already imports `@app/orders/module`, calls this once:
 *
 * ```ts
 * import { module as ordersModule, acceptHotUpdate } from '@app/orders/module';
...
export function acceptHotUpdate(
  ...
  hot.accept((next) => {
    const replacement = next as OrdersModuleExports | undefined;

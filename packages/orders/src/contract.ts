...
/** A not-yet-committed edit. Durable across HMR — see `OrdersDraftsToken`. */
...
export interface OrdersService {
  load(id: string): Promise<OrdersRecord>;
  on(event: string, handler: (record: OrdersRecord) => void): void;
  off(event: string, handler: (record: OrdersRecord) => void): void;
...
export const OrdersSettingsToken = createToken<OrdersSettings>('orders/Settings');
export const OrdersGatewayToken = createToken<OrdersGateway>('orders/Gateway');
export const OrdersServiceToken = createToken<OrdersService>('orders/Service');
export const OrdersSessionToken = createToken<OrdersSession>('orders/Session');
/** **H3**: durable state lives in a store, never in a module closure. */
export const OrdersDraftsToken =
  createToken<Store<readonly OrdersDraft[]>>('orders/Drafts');

...
//
// The hot block below is verbatim the one `create-module` emits (spec §17,
// issues #46/#42); its rationale is not repeated here. What matters for this
// package is that it is *ordinary* — the navigation module hot-replaces
// exactly like a feature module, with no kernel involvement.
 ...
 * lifecycle and provider chunks". It cannot: a kernel module id (`'nav'`)
 ...
 * the kernel and already imports `@app/nav/module`, calls this once:
 *
 * ```ts
 * import { module as ordersModule, acceptHotUpdate } from '@app/nav/module';
 * const kernel = createKernel({ modules: [ordersModule, …] });
...
export function acceptHotUpdate(kernel: Kernel, hot?: ModuleHotContext): void {
  ...
  context.accept((next) => {
    const replacement = next as NavModuleExports | undefined;
    ...
    void kernel.hotReplace(NavModule, replacement.module);

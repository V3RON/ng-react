...
Metro adapter left as a documented, trivial second implementation. Optional `invalidate` is
the sole capability signal: `{}` is the noop and Metro shape. **No kernel code may
...
> `accept`/`dispose`/`invalidate`/`enabled`; it is now **optional `invalidate` only**. `{}` is
> the noop and Metro shape. `accept` cannot live
